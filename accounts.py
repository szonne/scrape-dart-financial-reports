...
class CashFlowAccounts(Enum):
    ...
    PROFIT_LOSS = '당기순이익'
    ADJUSTMENTS_FOR_ASSETS_LIABILITIES_OF_OPERATING_ACTIVITIES = '영업으로부터 창출된 현금흐름'
    INTEREST_PAID = '이자지급'
...
def get_bs_account_detail(account: BalanceSheetAccounts):
    ...
    if account == BalanceSheetAccounts.CASH_AND_CASH_EQUIVALENTS:
        return {'names': ['현금및현금성자산'],
                'ids': ['ifrs-full_CashAndCashEquivalents', 'ifrs-full_Cash', 'ifrs-full_CashEquivalents']}
    if account == BalanceSheetAccounts.TRADE_AND_OTHER_CURRENT_RECEIVABLES:
        return {
            'names': ['매출채권', '매출채권및기타채권', '매출채권 및 기타수취채권', '매출채권 및 기타채권'],
            'ids': [
    ...
    if account == BalanceSheetAccounts.INVENTORIES:
        return {
            'names': ['재고자산'],
            'ids': ['ifrs-full_Inventories', 'dart_RawMaterialsGross', 'ifrs-full_InventoriesTotal'],
        }
    ...
    if account == BalanceSheetAccounts.TRADE_AND_OTHER_NON_CURRENT_RECEIVABLES:
        return {
            'names': [
                '매출채권(비유동)', '장기매출채권', '장기매출채권및기타채권', '장기매출채권 및 기타채권', '비유동매출채권 및 기타채권', '장기성매출채권',
    ...
    if account == BalanceSheetAccounts.PROPERTY_PLANT_AND_EQUIPMENT:
        return {
            'names': ['유형자산'],
            'ids': ['ifrs-full_PropertyPlantAndEquipment', 'dart_OtherPropertyPlantAndEquipmentGross']
        }
    ...
    if account == BalanceSheetAccounts.TRADE_AND_OTHER_CURRENT_PAYABLES:
        return {'names': ['매입채무', '유동매입채무', '매입채무및기타채무'],
                'ids': [
    ...
    if account == BalanceSheetAccounts.SHORT_TERM_BORROWINGS:
        return {
            'names': ['유동성장기차입금', '단기차입금', '단기차입금 및 유동성장기부채', '단기차입금및유동성장기차입금',
                      '외화단기차입금', '단기 차입금', '차입금', '유동 장기 차입금', '유동성 단기차입금',
                      '차입금및사채', '유동성사채및장기차입금', '유동차입금및유동사채',
                      '유동 차입금 및 사채', '유동성차입금', '유동차입금'
                      ],
            'ids': ['ifrs-full_ShorttermBorrowings', 'ifrs-full_Borrowings',
    ...
    if account == BalanceSheetAccounts.TRADE_AND_OTHER_NON_CURRENT_PAYABLES:
        return {
            'names': ['비유동매입채무 및 기타채무', '장기매입채무 및 기타채무', '매입채무 및 기타금융부채(비유동)', '장기매입채무및기타채무'],
            'ids': ['dart_LongTermTradeAndOtherNonCurrentPayables', 'dart_LongTermTradePayablesGross']
        }
    if account == BalanceSheetAccounts.LONG_TERM_BORROWINGS:
        return {
            'names': ['장기차입금 및 사채', '장기차입금', '비유동차입금및비유동사채', '비유동차입금'],
    ...
    if account == BalanceSheetAccounts.LONG_TERM_ADVANCES_CUSTOMERS:
        return {
            'names': ['장기선수금'],
            'ids': ['dart_LongTermAdvancesCustomers']
        }
    ...
    if account == BalanceSheetAccounts.RETAINED_EARNINGS:
        return {'names': ['이익잉여금', '이익잉여금(결손금)'], 'ids': ['ifrs-full_RetainedEarnings']}
...
def get_cis_account_detail(account: IncomeStatementAccounts):
    ...
    if account == IncomeStatementAccounts.REVENUE_FROM_SALE_OF_GOODS_PRODUCT:
        return {'names': ['제품매출', '제품매출액'],
                'ids': ['dart_RevenueFromSaleOfGoodsProduct', 'ifrs-full_RevenueFromSaleOfGoods']}
    if account == IncomeStatementAccounts.REVENUE_FROM_SALE_OF_GOODS_MERCHANDISE:
        return {'names': ['상품매출', '상품매출액', '(1) 상품매출액'], 'ids': ['dart_RevenueFromSaleOfGoodsMerchandise']}
    ...
    if account == IncomeStatementAccounts.COST_OF_SALES_FROM_SALE_OF_GOODS_PRODUCT:
        return {'names': ['제품매출원가'], 'ids': ['dart_CostOfSalesFromSaleOfGoodsProduct']}
    if account == IncomeStatementAccounts.COST_OF_SALES_FROM_SALE_OF_GOODS_MERCHANDISE:
        return {'names': ['상품매출원가'], 'ids': ['ifrs-full_CostOfMerchandiseSold', 'dart_CostOfSalesFromSaleOfGoods']}
    ...
    if account == IncomeStatementAccounts.OPERATING_INCOME_LOSS:
        return {'names': ['영업이익'], 'ids': ['dart_OperatingIncomeLoss', 'ifrs-full_ProfitLossFromOperatingActivities']}
    if account == IncomeStatementAccounts.PROFIT_LOSS:
        return {'names': ['당기순이익', '당기순이익(손실)'], 'ids': ['ifrs-full_ProfitLoss']}
...
def get_cf_account_detail(account: CashFlowAccounts):
    ...
    if account == CashFlowAccounts.ADJUSTMENTS_FOR_ASSETS_LIABILITIES_OF_OPERATING_ACTIVITIES:
        return {
            'names': ['영업으로부터 창출된 현금흐름', '영업에서 창출된 현금', '영업으로부터창출된현금', '영업에서 창출한 현금흐름', '영업활동에서창출된현금흐름'],
            'ids': [
    ...
    if account == CashFlowAccounts.PURCHASE_OF_FINANCIAL_INSTRUMENTS:
        return {
            'names': [
                '단기금융상품의 증가', '단기금융상품의 취득', '장단기금융상품의 취득', '금융상품의 증가', '장기금융상품의 납입', '장기금융상품의증가',
                '장단기금융상품의 증가', '장ㆍ단기금융상품의 증가', '장,단기금융상품의 증가'],
            'ids': [
    ...
    if account == CashFlowAccounts.SALES_OF_FINANCIAL_INSTRUMENTS:
        return {
            'names': [
                '단기금융상품의 감소', '장기금융상품의처분', '장기금융상품의 감소', '장단기금융상품의 처분', '단기금융상품의 처분', '장기금융상품의 해지', '단기금융상품의 해지',
                '장기금융상품의감소', '단기금융상품의감소', '장단기금융상품의 감소', '장ㆍ단기금융상품의 감소', '장,단기금융상품의 감소'
            ],
            'ids': [
    ...
    if account == CashFlowAccounts.PURCHASE_OF_PROPERTY_PLANT_AND_EQUIPMENT:
        return {
            'names': ['유형자산의 취득', '유형자산의취득', '유형자산의 증가', '유형자산 취득'],
            'ids': [
    ...
    if account == CashFlowAccounts.SALES_OF_PROPERTY_PLAND_AND_EQUIPMENT:
        return {
            'names': ['유형자산의 처분', '유형자산의 감소', '유형자산 처분', '유형자산 감소'],
            'ids': [
    ...
    if account == CashFlowAccounts.PROCEEDS_FROM_BORROWINGS:
        return {
            'names':
                [
                    '차입금의 증가', '단기차입금의 차입', '장기차입금의 차입', '장기차입금의 증가', '단기차입금의 증가',
                    '차입금의 순차입', '단기차입금의차입', '장기차입금의차입', '단기차입금의 순증감',
                    '장기차입금의 증감', '차입금의 차입', '단기차입금의 순차입', '장기차입금의 순차입',
                    '차입금및사채의 증가', '장기차입금 및 사채의 차입', '사채 및 장기차입금 차입',
                    '단기차입금의증가', '유동성장기차입금의 차입', '유동성장기차입금의 증가',
                    '장기차입금 차입', '단기차입금 증가', '차입금 차입', '유동성장기부채및단기차입금의 차입', '장기차입금의증가',
                    '단기차입금 및 사채의 차입', '유동성차입금의 증가', '차입금 및 사채의 차입'
                ],
            'ids': [
    ...
    if account == CashFlowAccounts.REPAYMENTS_OF_BORROWINGS:
        return {
            'names': [
                '유동성장기차입금의 상환', '장기차입금의 상환', '단기차입금의 상환', '유동성장기차입금의 감소', '장기차입금의 감소',
                '단기차입금의 감소', '유동장기차입금의 상환', '차입금의 상환', '차입금의 감소', '차입금의 순상환', '유동성장기차입금의상환',
                '유동성 장기차입금의 상환', '(유동성)장기차입금 상환', '유동성장기차입금 상환', '장기차입금의상환', '단기차입금의상환',
                '유동차입금의 상환', '장기차입금 상환', '차입금및사채의 상환', '유동성차입금의 상환', '장기차입금 및 사채의 상환',
                '사채 및 장기차입금 상환', '단기차입금의 순상환', '차입금 상환', '유동성단기차입금의 상환', '단기차입금의감소',
                '단기차입금 감소', '유동성장기부채및단기차입금의 상환', '유동성 장기차입금의 감소', '유동성장차입금의 감소',
                '단기차입금 및 사채의 상환', '유동성차입금(기타)의 상환', '유동성차입금의 감소', '유동성자기차입금 상환',
                '장기차입금 감소', '유동성장기차입금의감소',
            ],
            'ids': [
                'dart_RepaymentsOfLongTermBorrowings', 'dart_RepaymentsOfShortTermBorrowings',
